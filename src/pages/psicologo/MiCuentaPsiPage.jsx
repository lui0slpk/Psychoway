...
import { User, FileText, Lock, Eye, EyeOff, Save, Trash2, Camera, Loader } from "lucide-react";
...
function MiCuentaPsiPage() {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        const res = await authFetch(`${API_URL}/api/users/profile/${user.id || user.id_user}`);
        if (res.ok) {
  ...
  const handlePhotoChange = (e) => {
    ...
    reader.onload = async (event) => {
      ...
      try {
        const res = await authFetch(`${API_URL}/api/users/profile-photo/${user.id || user.id_user}`, {
          method: "PUT",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await authFetch(`${API_URL}/api/users/update/${user.id || user.id_user}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await authFetch(`${API_URL}/api/users/delete/${user.id || user.id_user}`, { method: "DELETE" });
          if (res.ok) {
  ...
  return (
    <MainLayout pageTitle="Mi Cuenta" pageSubtitle="Modifica tus datos de registro" currentPage="mi-cuenta-psi">
