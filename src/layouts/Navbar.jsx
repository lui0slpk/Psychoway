...
function Navbar({ pageTitle, pageSubtitle }) {
  const { user, logout, authFetch, profilePhoto, setProfilePhoto } = useAuth();
  ...
  useEffect(() => {
    ...
    authFetch(`http://localhost:5000/api/users/profile/${user.id || user.id_user}`)
            ...
            {profilePhoto ? (
              <img
                ...
                style={{
                  ...
                  border: "2px solid #e9ecef", marginRight: 6,
                }}
              />
            ) : (
              <i
