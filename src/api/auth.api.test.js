...
describe("authApi.login", () => {
  it("POSTs {document, password} to `${API_BASE}/login` (no /api) without Authorization", async () => {
    ...
    expect(options.body).toBe(JSON.stringify({ document: "123456", password: "secreto" }));
  });

  it("rejects ApiError(401) with the backend message and does NOT redirect on bad credentials", async () => {
    global.fetch.mockResolvedValue(
      jsonResponse({ message: "Documento o contraseña incorrectos" }, 401)
    );
  ...
  it("resolves the parsed {token, user} payload on success", async () => {
    global.fetch.mockResolvedValue(
      jsonResponse({ message: "Login exitoso", token: "abc", user: { id: 7, rol: "aprendiz" } })
    );
...
describe("authApi.register", () => {
  it("POSTs userData to `${API_BASE}/register` (no /api prefix)", async () => {
    ...
    global.fetch.mockResolvedValue(jsonResponse({ message: "Usuario registrado correctamente" }));
  ...
  it("rejects ApiError(409) on duplicate document/email so pages can branch on err.status", async () => {
    global.fetch.mockResolvedValue(
      jsonResponse({ message: "El documento o correo ya se encuentra registrado" }, 409)
    );

    const error = await authApi.register({ document: "123" }).catch((e) => e);

    expect(error).toBeInstanceOf(ApiError);
    expect(error.status).toBe(409);
    expect(error.message).toBe("El documento o correo ya se encuentra registrado");
...
describe("authApi.forgotPassword", () => {
  it("POSTs {correo} to `${API_URL}/password/forgot` without Authorization", async () => {
    global.fetch.mockResolvedValue(
      jsonResponse({ message: "Correo de recuperación enviado exitosamente" })
    );
  ...
  it("surfaces the backend 404 message for an unknown email", async () => {
    global.fetch.mockResolvedValue(jsonResponse({ message: "Correo no encontrado" }, 404));

    const error = await authApi.forgotPassword("nadie@mail.com").catch((e) => e);
...
describe("authApi.resetPassword", () => {
  it("POSTs {token, newPassword} to `${API_URL}/password/reset` without Authorization", async () => {
    global.fetch.mockResolvedValue(jsonResponse({ message: "Contraseña actualizada correctamente" }));
    ...
    expect(options.headers["Content-Type"]).toBe("application/json");
    expect(options.headers.Authorization).toBeUndefined();
    expect(options.body).toBe(JSON.stringify({ token: "tok-1", newPassword: "Nueva#Pass1" }));
  });

  it("surfaces the backend 400 message for an invalid token", async () => {
    global.fetch.mockResolvedValue(jsonResponse({ message: "Token inválido o expirado" }, 400));

    const error = await authApi.resetPassword("bad", "Nueva#Pass1").catch((e) => e);
...
describe("authApi.verifySession", () => {
  it("GETs `${API_URL}/auth/verify` with the Bearer token (protected)", async () => {
    ...
    global.fetch.mockResolvedValue(jsonResponse({ valid: true, userId: 3, role: "psicologo" }));
  ...
  it("keeps the DEFAULT 401 redirect (no skipAuthRedirect) for an expired session", async () => {
    ...
    global.fetch.mockResolvedValue(jsonResponse({ message: "Token expirado" }, 401));
